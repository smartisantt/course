...
class ServerAPI():
    ...
    def checkSumBuilder(self):
        ...
        self.CheckSum = hashlib.sha1(join_string.encode("utf8")).hexdigest()
    ...
    def postDataHttps(self, url, data):
        ...
        req = urllib.request.Request(url, urllib.parse.urlencode(postdata).encode(), headers=headers)
    ...
    def sendMsg(self, fromx, ope, to, typex, body, option=dict
        ({"push" :'false', "roam" :'true', "history" :'false', "sendersync" :'true', "route" :'false'}), pushcontent=''):
     ...
     * @return result      [返回python dict 对象]
    '''
    def createGroup(self, tname, owner, members, announcement='', intro='', msg='', magree='0', joinmode='0'
                   , custom='0'):
    ...
    '''
     * 创建聊天室
     https://dev.yunxin.163.com/docs/product/IM%E5%8D%B3%E6%97%B6%E9%80%9A%E8%AE%AF/%E6%9C%8D%E5%8A%A1%E7%AB%AFAPI%E6%96%87%E6%A1%A3/%E8%81%8A%E5%A4%A9%E5%AE%A4
     * @param  creator   [聊天室属主的账号accid]
     * @param  name      [聊天室名称，长度限制128个字符]
     * @return result    [返回python dict 对象]
    '''
    def createChatroom(self, creator, name, announcement="", broadcasturl="", ext="", queuelevel=0):
     ...
     * @return result    [返回python dict 对象]
    '''
    def getChatroomInfo(self, roomid, needOnlineUserCount=False):
     ...
     * 设置聊天室内用户角色
    '''
    def setChatroomRole(self, roomid, operator, target, opt, optvalue, notifyExt='{}'):
     ...
     * 请求聊天室地址
    '''
    def getChatroomUrl(self, roomid, accid, clienttype=1, clientip=""):
     ...
     * 批量获取在线成员信息
    '''
    def getChatroomOnlineUserInfo(self, roomid, accids):
     ...
     * 发送聊天室消息
    '''
    def chatroomSendMsg(self, roomid, msgId, fromAccid, msgType, attach):
     ...
     * 往聊天室内添加机器人
    '''
    def chatroomAddRobot(self, roomid, accids, fromAccid, msgType, attach):
...
if __name__ == '__main__':
    api = ServerAPI("6f5ab152e315ac02305dec06cc3c1be5", "d21dba26e1f1")
    # 创建云信ID
    # api.createUserId("12321")

    # 创建云信ID 181个
    # for i in range(181):
    #     # 666666000  ~  666666180
    #     api.createUserId("666666"+str(i).rjust(3, '0'))

    # 更新云信ID
    # for i in range(181):
    #     # 666666000  ~  666666180
    #     api.updateUserId("666666"+str(i).rjust(3, '0'), token=("666666"+str(i).rjust(3, '0')))

    # 更新并获取新token
    # api.updateUserToken("12321")
    # 获取用户名片
    # api.getUinfos(["12321"])
    # 更新用户名片
    # api.updateUinfo("12321", name="ququ",)
    # 账号全局禁言
    # api.mute("12321", False)

    # 创建聊天室
    # api.createChatroom("12321", "测试聊天室")
    # {'chatroom': {'roomid': 138460065, 'valid': True, 'announcement': None, 'queuelevel': 0,
    # 'muted': False, 'name': '测试聊天室', 'broadcasturl': None, 'ext': '', 'creator': '12321'}, 'code': 200}
    ...
    # 查询聊天室信息
    # api.getChatroomInfo("138460065")
    # 设置聊天室内用户角色
    # api.setChatroomRole(138460065, "12321", "12322", 2, True)

    # 请求聊天室地址
    # api.getChatroomUrl(138460065, "12321")  # weblinkoutsea02.netease.im:443

    # 批量获取在线成员信息
    # api.getChatroomOnlineUserInfo(138460065, ["12321"])

    # 发送聊天室消息
    # api.chatroomSendMsg(138460065, "1", "12321", 0, {"msg": "Hello"})
